...
#   mldf = mlogit.data(trainData, choice="School", shape="wide", varying = 18:87)
#   mldf$Enrol = ifelse(mldf$Level == "UG", mldf$UG, ifelse(mldf$Level == "Grad", mldf$Grad, mldf$Total))
#   test_mldf = mlogit.data(testData, choice="School", shape="wide", varying = 18:87)
#   test_mldf$Enrol = ifelse(test_mldf$Level == "UG", test_mldf$UG, ifelse(test_mldf$Level == "Grad", test_mldf$Grad, test_mldf$Total))
#   model = mlogit(School ~ Dist + Family:Dist + Enrol | 0, data=mldf, reflevel="SG")
#   actuals = testData$School
