...
for (i in 0:(num_segments-1))
{
  ...
  if (i == 0)
  {
    ref = mlogit(Campus ~ get(paste("S", i, sep="")) | 0, data=ml_data, reflevel="YG")
    enr = mlogit(Campus ~ Total | 0, data=ml_data, reflevel="YG")
    dst = mlogit(Campus ~ Dist | 0, data=ml_data, reflevel="YG")
    end = mlogit(Campus ~ Dist + Total | 0, data=ml_data, reflevel="YG")
    grv = mlogit(Campus ~ Dist, data=ml_data, reflevel="YG")
  }
  else if (i < 4) 
  {
    ref = mlogit(Campus ~ get(paste("S", i, sep="")) | 0, data=ml_data, reflevel="YG")
    enr = mlogit(Campus ~ UG | 0, data=ml_data, reflevel="YG")
    dst = mlogit(Campus ~ Dist | 0, data=ml_data, reflevel="YG")
    end = mlogit(Campus ~ Dist + UG | 0, data=ml_data, reflevel="YG")
    grv = mlogit(Campus ~ Dist, data=ml_data, reflevel="YG")
  }
  else
  {
    ref = mlogit(Campus ~ get(paste("S", i, sep="")) | 0, data=ml_data, reflevel="YG")
    enr = mlogit(Campus ~ Grad | 0, data=ml_data, reflevel="YG")
    dst = mlogit(Campus ~ Dist | 0, data=ml_data, reflevel="YG")
    end = mlogit(Campus ~ Dist + Grad | 0, data=ml_data, reflevel="YG")
    grv = mlogit(Campus ~ Dist, data=ml_data, reflevel="YG")
  }
