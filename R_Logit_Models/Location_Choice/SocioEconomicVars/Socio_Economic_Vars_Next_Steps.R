...
for (call_idx in 1:length(formulas)) {
  ...
  # ---- Reference Model ----
  model1 = mlogit(School_Codes ~ Dist + Family:Dist + Level:Dist + Status:Dist + Family:Level:Dist + Family:Status:Dist + Level:Status:Dist | Family + Level, data=ml_data, reflevel="SG", subset = ml_data$Segment != 0, weights = ml_data$Exp_Segment)
