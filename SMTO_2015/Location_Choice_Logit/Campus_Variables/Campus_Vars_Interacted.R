...
mldf$Income_High = ifelse(mldf$Income == 'High', 1, 0)
mldf$Income_Low = ifelse(mldf$Income == 'Low', 1, 0)
mldf$Income_Unknown = ifelse(mldf$Income == 'Unknown', 1, 0)
mldf$NonFam = 1 - mldf$Family
mldf$Admission = mldf$Admission * 100
mldf$Domestic_UG = mldf$Domestic_UG * 100

# Distance Model ----
model = mlogit(School ~ Dist + log(Total) + Admission + Domestic_UG:Family | 0, data=mldf)
summary(model)
...
# Accessibility Model ----
model = mlogit(School ~ Access + log(Total) + Admission + Domestic_UG:Family | 0, data=mldf)
summary(model)
probs = get_probs(model)
hard_preds = hardmax_preds(probs, real_levels)
hard_cm = get_cm(hard_preds, actuals)
...
# Combined Model ----
model = mlogit(School ~ Dist + Access + log(Total) + Admission + Domestic_UG:Family | 0, data=mldf)
summary(model)
probs = get_probs(model)
hard_preds = hardmax_preds(probs, real_levels)
hard_cm = get_cm(hard_preds, actuals)
