...
df <- read.csv("../../../Data/SMTO_2015/SMTO_2015_Complete_Input_withPD.csv")
df$School = as.factor(df$School)
mldf = mlogit.data(df, choice="School", shape="wide", varying = 19:88)
...
for (zone_idx in 1:length(PD)){
#for (zone_idx in 1:length(hz)){
  print('PD:')
  print(PD[zone_idx])
  print('')
  temp_df = subset(df, Segment!=0,)[subset(df, Segment!=0)$PD == PD[zone_idx],] # --- Get 'temp_df': subset of observations from specific zone --- (Segment 0 is always removed)
  #temp_df = subset(df, Segment!=0)[subset(df, Segment!=0)$HomeZone == hz[one_idx],]
  ...
  else{   # if we have more than one observation, then R will keep the indices as a normal dataframe, so temp_probs is just = probs[indices,]   
  ...
  for (i in 1:length(campuses)){ 
    #print(campuses[i])
    z = temp_probs[which(temp_df$School == campuses[i]),] # z is the subset of rows from our temp_probs where the student goes to the specified school
  ...
  residuals[zone_idx,1] = PD[zone_idx]
  print('Almost There')
  for (i in 1:length(campuses)){
    residuals[zone_idx,i+1] = (pred_obs[1,i] - pred_obs[2,i])/rowSums(pred_obs)[1]
  }
  print('ROUND FINISHED')
}

#write.table(residuals, '../Proposed/Residuals_All_PDs.csv', sep = ",", col.names = c('Zone', campuses))
